...
%   converge to the covariance-based average results 
%
%INPUTS & OUTPUTS:
...
function RobotArray = DAC(RobotArray)
...
% time-varying
    ...
    % initilize the consensus state of all robots
    if RobotArray(1,1).distributd_flag == 0
        for i = 1:robot_num
            RobotArray(1,i).rtheta = RobotArray(1,i).sigma.^(-1).*RobotArray(1,i).mu;
            RobotArray(1,i).rsigma = RobotArray(1,i).sigma;
            RobotArray(1,i).rs = RobotArray(1,i).sigma.^(-1);
            
            RobotArray(1,i).theta = RobotArray(1,i).mu;
            RobotArray(1,i).Dist_mu = RobotArray(1,i).sigma.^(-1).*RobotArray(1,i).mu;
        ...
        for i = 1:robot_num
            ...
            theta = [theta RobotArray(1,i).theta];
            rtheta = [rtheta RobotArray(1,i).sigma.^(-1).*RobotArray(1,i).mu];
        ...
        % update DAC, 0.7 is the steplength
        ...
        for i = 1:robot_num
            ...
            RobotArray(1,i).theta = theta(:,i);
            RobotArray(1,i).Dist_mu = sinv(:,i).^(-1).*theta(:,i);
